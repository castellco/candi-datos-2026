...
crear_labels_unicos <- function(nombres, largo = 25) {
  ...
  if (any(dupes)) {
    ...
    for (grp in idx_list) for (k in seq_along(grp))
...
DT_OPCIONES <- list(
  ...
  rowCallback  = JS("function(row,data){$(row).css({'background-color':'#383838','color':'#ffffff'});$('td',row).css({'border-color':'#464743'});}"),
  initComplete = JS("function(settings,json){var api=this.api();$(api.table().header()).css({'background-color':'#464743','color':'#ffffff','border-bottom':'2px solid #be2e1c'});$(api.table().node()).css('background-color','#383838');$(api.table().container()).find('.dataTables_info,.dataTables_paginate,.paginate_button').css({'color':'#ffffff','background-color':'#383838'});}"),
  drawCallback = JS("function(settings){$('.dataTables_paginate,.dataTables_info').css({'color':'#ffffff'});$('.paginate_button').css({'color':'#ffffff !important'});}"),
  language     = list(url = "//cdn.datatables.net/plug-ins/1.10.11/i18n/Spanish.json")
)

# ── Carga de datos con readr (sin duckdb ni DBI) ──────────────────────────────
leer <- function(nombre) {
  read_csv(file.path("data", paste0(nombre, ".csv")),
           show_col_types = FALSE, name_repair = "universal") |> clean_names()
}
...
sentencias_df   <- leer("sentencias")
exp_laboral_raw <- leer("experiencia_laboral")

exp_anos_por_dni <- exp_laboral_raw |>
  mutate(anio_inicio = suppressWarnings(as.numeric(str_extract(exp_laboral_periodo, "^\\d{4}"))),
...
cargos_presidenciales <- cargos_list[str_detect(cargos_list, regex("president|vicepres", ignore_case = TRUE))]
...
ui <- page_navbar(
  ...
  sidebar = sidebar(width = 260, bg = "#1e1e1e",
    ...
    tags$b("Filtra por:", style = "color:#ffffff;"), br(),
    selectInput("dep",     "Departamento", choices = c("Todos" = "__ALL__", deps_list),     selected = "__ALL__"),
    selectInput("partido", "Partido",      choices = c("Todos" = "__ALL__", partidos_list), selected = "__ALL__"),
    selectInput("cargo",   "Cargo",        choices = c("Todos" = "__ALL__", cargos_list),   selected = "__ALL__"),
    checkboxGroupInput("sexo", "Sexo", choices = sexos_list, selected = sexos_list, inline = TRUE),
...
server <- function(input, output, session) {
  ...
  vbox <- function(title, value)
    bslib::value_box(title=title, value=value, theme=value_box_theme(bg=rojo, fg=blanco))
  ...
  output$plot_ingresos_partido <- renderPlotly({
    ...
    plot_ly(datos, x=~promedio, y=~reorder(partido,promedio),
      customdata=~partido, source="src_ingresos",
      type="bar", orientation="h", marker=list(color=rojo),
      text=~paste0(partido,"<br>Promedio: ",etiqueta,"<br>Candidatos: ",n_cand,
                   "<br>Total: ",dollar(total,prefix="S/ ",big.mark=",",accuracy=1)),
      hoverinfo="text", texttemplate=~etiqueta, textposition="outside", cliponaxis=FALSE,
      textfont=list(color=blanco,size=12,family="Cabin")
    ) |> layout(
      title=titulo_plotly("Ingresos promedio por partido", paste0("En S/ · ",n_label(d))),
      xaxis=list(color=blanco,gridcolor=gris_claro,tickformat=",.0f",tickprefix="S/ ",tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title=""),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=15,t=90,b=5)
  ...
  output$plot_sentencias_partido <- renderPlotly({
    ...
    plot_ly(datos, x=~n, y=~reorder(partido,n),
      customdata=~partido, source="src_sentencias",
      type="bar", orientation="h", marker=list(color=rojo),
      text=~paste0(partido,"<br>Con sentencias: ",n," de ",total),
      hoverinfo="text", texttemplate=~as.character(n), textposition="outside", cliponaxis=FALSE,
      textfont=list(color=blanco,size=12,family="Cabin")
    ) |> layout(
  ...
  output$plot_ingreso_tipo <- renderPlotly({
    base <- fc_uniq() |>
      summarise(publico=sum(rem_publico_num,na.rm=TRUE), privado=sum(rem_privado_num,na.rm=TRUE), .by=partido) |>
      filter(publico+privado>0) |>
      mutate(total=publico+privado, pct_pub=round(publico/total*100,1), pct_priv=round(privado/total*100,1)) |>
      arrange(pct_pub)
    ...
    plot_ly(largo|>filter(tipo=="pct_pub"), x=~pct, y=~partido, name="Público",
      type="bar", orientation="h", marker=list(color=rojo),
      text=~tooltip, hoverinfo="text", texttemplate=~paste0(pct,"%"), textposition="auto",
      textfont=list(color=blanco,size=12,family="Cabin")
    ) |> add_trace(data=largo|>filter(tipo=="pct_priv"), x=~pct, y=~partido, name="Privado",
      marker=list(color=amarillo), text=~tooltip, hoverinfo="text",
      texttemplate=~paste0(pct,"%"), textposition="auto",
      textfont=list(color=background_oscuro,size=12,family="Cabin")
    ) |> layout(barmode="stack",
      title=titulo_plotly("Origen de ingresos por partido", paste0("% público vs. privado · ",n_label(fc_uniq()))),
      xaxis=list(color=blanco,gridcolor=gris_claro,ticksuffix="%",range=c(0,100),tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title="",
                 categoryorder="array", categoryarray=base$partido[order(base$pct_pub)]),
      legend=list(orientation="h",x=0.5,xanchor="center",y=1.08,font=list(color=blanco,family="Cabin")),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=15,t=110,b=5)
    )
  })

  output$plot_sexo_partido <- renderPlotly({
    base <- fc_uniq() |> filter(!is.na(sexo)) |>
      count(partido,sexo,name="n") |> complete(partido,sexo,fill=list(n=0)) |>
      group_by(partido) |>
      mutate(total=sum(n), pct=if_else(total>0,round(n/total*100,1),0)) |> ungroup()
    validate(need(nrow(base)>0,"Sin datos para mostrar."))
    plot_ly(base|>filter(sexo=="Femenino"), x=~pct, y=~partido, name="Femenino",
      type="bar", orientation="h", marker=list(color=rojo),
      text=~paste0(partido,"<br>Femenino: ",pct,"%"), hoverinfo="text",
      texttemplate=~paste0(pct,"%"), textposition="auto",
      textfont=list(color=blanco,size=12,family="Cabin")
    ) |> add_trace(data=base|>filter(sexo=="Masculino"), x=~pct, y=~partido, name="Masculino",
      marker=list(color=blanco), text=~paste0(partido,"<br>Masculino: ",pct,"%"), hoverinfo="text",
      texttemplate=~paste0(pct,"%"), textposition="auto",
      textfont=list(color=background_oscuro,size=12,family="Cabin")
    ) |> layout(barmode="stack",
      ...
      xaxis=list(color=blanco,gridcolor=gris_claro,ticksuffix="%",range=c(0,100),tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title="",
                 categoryorder="array", categoryarray=(base|>filter(sexo=="Femenino")|>arrange(pct))$partido),
      legend=list(orientation="h",x=0.5,xanchor="center",y=1.08,font=list(color=blanco,family="Cabin")),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=15,t=110,b=5)
    )
  ...
  output$plot_exp_laboral <- renderPlotly({
    ...
    datos <- d |>
      ...
      arrange(promedio_exp) |> mutate(etiqueta=round(promedio_exp,1))
    plot_ly(datos, x=~promedio_exp, y=~reorder(partido,promedio_exp),
      type="bar", orientation="h", marker=list(color=rojo),
      text=~paste0(partido,"<br>Promedio: ",etiqueta," años<br>Candidatos: ",n_cand),
      hoverinfo="text", texttemplate=~paste0(etiqueta," años"), textposition="outside", cliponaxis=FALSE,
      textfont=list(color=blanco,size=12,family="Cabin")
    ) |> layout(
      title=titulo_plotly("Promedio de años de experiencia laboral por partido",
                          paste0(n_label(d)," con experiencia declarada")),
      xaxis=list(color=blanco,gridcolor=gris_claro,ticksuffix=" años",tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title=""),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=80,t=90,b=5)
    )
  })
  ...
  output$tabla_partidos <- DT::renderDT({
    ...
    d |> summarise(
      Candidatos=n_distinct(dni), Hombres=n_distinct(dni[sexo=="Masculino"]), Mujeres=n_distinct(dni[sexo=="Femenino"]),
      `Sin ed. básica`=sum(tiene_educacion_basica==0|is.na(tiene_educacion_basica)),
      `Sin estudios sup.`=sum((ed_universitaria==0|is.na(ed_universitaria))&(ed_tecnica==0|is.na(ed_tecnica))&
                               (ed_no_univ==0|is.na(ed_no_univ))&(ed_posgrado==0|is.na(ed_posgrado))),
      `Prom. años exp.`=round(mean(total_anos_exp,na.rm=TRUE),1),
      `Con sentencias`=sum(sentencias==1,na.rm=TRUE),
      `Ingreso promedio`=mean(total_ingresos_num,na.rm=TRUE), .by=partido
    ) |> arrange(desc(`Ingreso promedio`)) |> rename(Partido=partido) |>
      DT::datatable(rownames=FALSE, options=modifyList(DT_OPCIONES,list(pageLength=-1,paging=FALSE))) |>
  ...
  output$plot_ranking_candidatos_sent <- renderPlotly({
    ...
    datos <- sentencias_df |> filter(dni %in% dnis_fc) |>
      group_by(dni) |>
      summarise(n_sent=n(),
                materias=paste(unique(sentencia_materia[!is.na(sentencia_materia)]),collapse="<br>· "),
                fallos=paste(sentencia_fallo[!is.na(sentencia_fallo)],collapse="<br>· "), .groups="drop") |>
      left_join(candidatos|>distinct(dni,.keep_all=TRUE)|>select(dni,nombre,partido,cargo_al_que_postula),by="dni") |>
      arrange(desc(n_sent)) |> slice_head(n=20) |>
      mutate(label_eje=paste0(str_trunc(nombre,25)," (",str_trunc(partido,12),")"),
             tooltip=paste0("<b>",nombre,"</b><br>",partido,"<br>",cargo_al_que_postula,
                            "<br><br><b>",n_sent," sentencia(s)</b><br><b>Materias:</b><br>· ",materias,
                            "<br><b>Fallos:</b><br>· ",fallos)) |> arrange(n_sent)
    validate(need(nrow(datos)>0,"Sin candidatos con sentencias en la selección."))
    plot_ly(datos, x=~n_sent, y=~reorder(label_eje,n_sent),
      type="bar", orientation="h", marker=list(color=rojo),
      text=~tooltip, hoverinfo="text", texttemplate=~as.character(n_sent),
      textposition="outside", cliponaxis=FALSE, textfont=list(color=blanco,size=12,family="Cabin")
    ) |> layout(
      title=titulo_plotly("Candidatos/as con más sentencias","Top 20 · cursor encima para más info"),
      xaxis=list(color=blanco,gridcolor=gris_claro,dtick=1,tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title=""),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=15,t=90,b=5)
    )
  ...
  output$plot_ranking_ingresos_cand <- renderPlotly({
    d <- fc_uniq() |> filter(!is.na(total_ingresos_num),total_ingresos_num>0) |>
      arrange(desc(total_ingresos_num)) |> slice_head(n=25)
    validate(need(nrow(d)>0,"Sin datos de ingresos."))
    label_n <- n_label(d)
    datos <- d |>
      mutate(etiqueta=dollar(total_ingresos_num,prefix="S/ ",big.mark=",",accuracy=1),
             label_eje=str_trunc(nombre,30)) |> arrange(total_ingresos_num)
    plot_ly(datos, x=~total_ingresos_num, y=~reorder(label_eje,total_ingresos_num),
      type="bar", orientation="h", marker=list(color=blanco),
      text=~paste0(nombre,"<br>",partido,"<br>",cargo_al_que_postula,"<br>Ingresos: ",etiqueta),
      hoverinfo="text", texttemplate=~etiqueta, textposition="outside", cliponaxis=FALSE,
      textfont=list(color=blanco,size=12,family="Cabin")
    ) |> layout(
      title=titulo_plotly("Candidatos con mayores ingresos declarados",paste0("Top 25 · ",label_n)),
      xaxis=list(color=blanco,gridcolor=gris_claro,tickformat=",.0f",tickprefix="S/ ",tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title=""),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=15,t=90,b=5)
  ...
  output$plot_ranking_sentencias <- renderPlotly({
    dnis_fc <- unique(fc()$dni)
    datos <- sentencias_df |> filter(dni %in% dnis_fc, !is.na(sentencia_materia)) |>
      count(sentencia_materia,name="n") |> arrange(n) |> slice_tail(n=10)
    validate(need(nrow(datos)>0,"Sin sentencias en la selección actual."))
    plot_ly(datos, x=~n, y=~reorder(sentencia_materia,n),
      type="bar", orientation="h", marker=list(color=rojo),
      text=~paste0(sentencia_materia,": ",n," sentencias"), hoverinfo="text",
  ...
  output$plot_treemap <- renderPlotly({
    ...
    datos <- d |>
      ...
      mutate(label=paste0("<b>",sexo,"</b><br>",dollar(promedio,prefix="S/ ",big.mark=",",accuracy=1),
                          "<br>",comma(n)," candidatos"))
    plot_ly(data=datos, labels=~sexo, parents=~"", values=~promedio,
      type="treemap", text=~label, hoverinfo="text", textinfo="text",
      marker=list(colors=c(rojo,blanco), line=list(color=background_oscuro,width=3)),
      pathbar=list(visible=FALSE)
    ) |> layout(
      title=list(text=paste0("<b>Ingreso promedio por sexo</b><br><sup>",n_label(d),"</sup>"),
                 font=list(color=blanco,family="Cabin",size=18),x=0.5,xanchor="center"),
      paper_bgcolor=background_oscuro, font=list(color=blanco,family="Cabin"),
  ...
  output$plot_sin_trayectoria <- renderPlotly({
    datos <- fc_uniq() |>
      filter((exp_laboral==0|is.na(exp_laboral)),(cargos_partidarios==0|is.na(cargos_partidarios)),
             (eleccion_popular==0|is.na(eleccion_popular)),!is.na(total_ingresos_num),total_ingresos_num>0) |>
      arrange(desc(total_ingresos_num)) |> slice_head(n=10) |>
      mutate(etiqueta=dollar(total_ingresos_num,prefix="S/ ",big.mark=",",accuracy=1),
             label_eje=str_trunc(nombre,30)) |> arrange(total_ingresos_num)
    ...
    plot_ly(datos, x=~total_ingresos_num, y=~reorder(label_eje,total_ingresos_num),
      ...
      title=titulo_plotly("Sin trayectoria declarada, pero con altos ingresos",
                          "Top 10 sin exp. laboral, cargos partidarios ni elección popular ante el JNE"),
      xaxis=list(color=blanco,gridcolor=gris_claro,tickformat=",.0f",tickprefix="S/ ",tickfont=list(family="Cabin")),
      yaxis=list(color=blanco,tickfont=list(family="Cabin",size=12),tickmode="linear",tick0=0,dtick=1,automargin=TRUE,title=""),
      paper_bgcolor=background_oscuro, plot_bgcolor=background_oscuro,
      font=list(color=blanco,family="Cabin"), margin=list(l=5,r=15,t=100,b=5)
    )
  })
  ...
  output$tabla_caption <- renderUI({
    ...
    if (!is.null(activo))
      tags$span(tags$span(paste0("Filtrado por: ",activo), style="color:#febd17;font-style:italic;"),
                tags$span(" · Cambia el filtro 'Partido' en la barra lateral para ver todos",
                          style="color:#aaaaaa;font-size:0.85rem;"))
    else tags$span("Tabla de candidatos", style="color:#ffffff;")
  })

  output$tabla_candidatos <- DT::renderDT({
    d <- fc_tabla() |>
      transmute(Nombre=nombre, DNI=dni, Cargo=cargo_al_que_postula, Partido=partido,
                Departamento=departamento, Sexo=sexo, `Ed. Básica`=tiene_educacion_basica,
                `Ed. Técnica`=ed_tecnica, `Ed. No Univ.`=ed_no_univ, `Ed. Univ.`=ed_universitaria,
                `Ed. Posgrado`=ed_posgrado, `Exp. Laboral`=exp_laboral,
                `Cargos Part.`=cargos_partidarios, `Elec. Popular`=eleccion_popular,
                Sentencias=sentencias, Bienes=bienes, `Total Ingresos`=total_ingresos_num,
                Ficha=ifelse(!is.na(url)&url!="",
                             sprintf('<a href="%s" target="_blank" style="color:#febd17">Ver ficha</a>',url),"—"))
    DT::datatable(d, escape=FALSE, rownames=FALSE, options=DT_OPCIONES) |>
      DT::formatCurrency("Total Ingresos",currency="S/ ",interval=3,mark=",",digits=0)
  })

  for (id in c("plot_ingresos_partido","plot_sentencias_partido","plot_ingreso_tipo",
               "plot_sexo_partido","plot_exp_laboral","plot_ranking_candidatos_sent",
               "plot_ranking_ingresos_cand","plot_ranking_sentencias","plot_treemap",
               "plot_sin_trayectoria","tabla_partidos","tabla_candidatos"))
