---
title: "CandiDATOS"
subtitle: " ·   Rumbo a las Elecciones Generales del Perú, 2026"
format:
  dashboard:
...
oscuro <- function(p, t = 80) {
  p |> layout(
    ...
    yaxis  = list(color = blanco, gridcolor = "transparent",
                  tickfont = list(family = "Cabin", size = 13), tickmode = "linear", tick0 = 0, dtick = 1, automargin = TRUE, title = ""),
...
titulo_plotly <- function(texto, subtitulo = NULL) {
  txt <- if (!is.null(subtitulo))
    ...
    paste0("<b>", texto, "</b>")
  list(text = txt, font = list(color = blanco, family = "Cabin", size = 20),
...
Información declarada por los/as candidatos/as.
...
<br>**Carolina Cornejo Castellano** </br>.
...
renderVboxes <- function(sufijo) {
  output[[paste0("vb_total",       sufijo)]] <- renderUI(vbox("Candidatos",        n_distinct(fc()$dni)))
  output[[paste0("vb_diputados",   sufijo)]] <- renderUI(vbox("Diputados/as",      n_distinct(fc()$dni[fc()$cargo_al_que_postula == "Diputados"])))
  output[[paste0("vb_senadores",   sufijo)]] <- renderUI(vbox("Senadores/as",      n_distinct(fc()$dni[fc()$cargo_al_que_postula == "Senadores"])))
  output[[paste0("vb_andino",      sufijo)]] <- renderUI(vbox("Parlamento Andino", n_distinct(fc()$dni[fc()$cargo_al_que_postula == "Parlamento Andino"])))
  output[[paste0("vb_presidencia", sufijo)]] <- renderUI(vbox("Presidencia",       n_distinct(fc()$dni[fc()$cargo_al_que_postula %in% cargos_presidenciales])))
  output[[paste0("vb_partidos",    sufijo)]] <- renderUI(vbox("Partidos",          n_distinct(fc()$partido)))
  output[[paste0("vb_sentencias",  sufijo)]] <- renderUI(vbox("Con sentencias",    n_distinct(fc()$dni[fc()$sentencias == 1])))
}
...
output$plot_ingresos_partido <- renderPlotly({
  ...
  datos <- d |>
    ...
    arrange(promedio) |>
    mutate(label_eje = label_partido[partido],
  ...
  plot_ly(datos,
    ...
    hoverinfo    = "text",
    texttemplate = ~etiqueta,
    textposition = "outside", cliponaxis = FALSE,
    textfont     = list(color = blanco, size = 13, family = "Cabin")
  ) |>
  layout(
    title = titulo_plotly("Según lo que declaran, ¿cuáles son los ingresos promedio por partido?",
                          paste0("En soles (S/) · ", n_label(d))),
...
# ── 2. Candidatos con sentencias por partido ──────────────────────────────────
...
output$plot_sentencias_partido <- renderPlotly({
  ...
  plot_ly(datos,
    ...
    hoverinfo    = "text",
    texttemplate = ~as.character(n),
    textposition = "outside", cliponaxis = FALSE,
    textfont     = list(color = blanco, size = 13, family = "Cabin")
  ) |>
  layout(
    title = titulo_plotly("¿Qué partidos tienen más candidatos con sentencias?", n_label(d)),
...
# ── 3. FIXME → CORREGIDO: Stacked 100% público vs privado por partido ─────────
# Problema anterior: variable shadowing (d reusada) + add_trace sin datos explícitos.
# Solución: pivot_longer para tener datos en formato largo y un solo plot_ly con color.
...
output$plot_ingreso_tipo <- renderPlotly({
  base <- fc_uniq() |>
    ...
    mutate(
      ...
      label_eje = label_partido[partido]
    ) |>
    arrange(pct_pub)

  validate(need(nrow(base) > 0, "Sin datos de remuneraciones en la selección."))

  # Formato largo: una fila por partido × tipo
  largo <- base |>
    pivot_longer(cols = c(pct_pub, pct_priv),
                 names_to = "tipo", values_to = "pct") |>
    mutate(
      tipo_label = if_else(tipo == "pct_pub", "Público", "Privado"),
      color      = if_else(tipo == "pct_pub", rojo, blanco),
      tooltip    = paste0(partido, "<br>", tipo_label, ": ", pct, "%")
    )

  # Orden del eje y: categoryorder en lugar de factor() que plotly no procesa
  nivel_orden <- base$label_eje
  label_n_pub <- n_label(fc_uniq())

  plot_ly(largo |> filter(tipo == "pct_pub"),
    x = ~pct, y = ~partido,
    name = "Público", type = "bar", orientation = "h",
    marker = list(color = rojo),
    text = ~tooltip, hoverinfo = "text",
    texttemplate = ~paste0(pct, "%"),
    textposition = "auto",
    textfont = list(color = blanco, size = 13, family = "Cabin")
  ) |>
  add_trace(
    ...
    texttemplate = ~paste0(pct, "%"),
    textposition = "auto",
    textfont = list(color = background_oscuro, size = 13, family = "Cabin")
  ) |>
  layout(
    barmode = "stack",
    uniformtext = list(minsize = 11, mode = "hide"),
    title = titulo_plotly("Origen de ingresos por partido",
                          paste0("% del total declarado · público vs. privado · ",
                                 label_n_pub)),
...
# ── 4. FIXME → CORREGIDO: Stacked 100% distribución por sexo por partido ──────
# Problema anterior: factor(label_eje, levels = orden) fallaba cuando algún partido
# no tenía ambos sexos. Solución: pivot_longer con complete() para rellenar huecos.

output$plot_sexo_partido <- renderPlotly({
  base <- fc_uniq() |>
    filter(!is.na(sexo)) |>
    count(partido, sexo, name = "n") |>
    # Garantizar que cada partido tenga fila para cada sexo (relleno con 0)
  ...
  orden       <- base |> filter(sexo == "Femenino") |> arrange(pct) |> pull(label_eje)
  label_n_sex <- n_label(fc_uniq())

  base_fem  <- base |> filter(sexo == "Femenino")
  base_masc <- base |> filter(sexo == "Masculino")

  plot_ly(base_fem,
    x = ~pct, y = ~partido,
    name = "Femenino", type = "bar", orientation = "h",
    marker = list(color = rojo),
  ...
  layout(
    ...
    title = titulo_plotly("Distribución por sexo por partido",
                          paste0("% de candidatos · ordenado por % femenino · ",
                                 label_n_sex)),
    xaxis = list(color = blanco, gridcolor = gris_claro, ticksuffix = "%",
                 range = c(0, 100), tickfont = list(family = "Cabin")),
    yaxis = list(color = blanco, gridcolor = "transparent",
                 tickfont = list(family = "Cabin", size = 13), tickmode = "linear", tick0 = 0, dtick = 1, automargin = TRUE, title = "",
                 categoryorder = "array", categoryarray = (base |> dplyr::filter(sexo == "Femenino") |> dplyr::arrange(pct))$partido),
    legend = list(orientation = "h", x = 0.5, xanchor = "center", y = 1.08,
                  font = list(color = blanco, family = "Cabin")),
    paper_bgcolor = background_oscuro, plot_bgcolor = background_oscuro,
    font = list(color = blanco, family = "Cabin"),
...
output$plot_exp_laboral <- renderPlotly({
  ...
  plot_ly(datos,
    ...
    hoverinfo    = "text",
    texttemplate = ~paste0(etiqueta, " años"),
    textposition = "outside", cliponaxis = FALSE,
    textfont     = list(color = blanco, size = 13, family = "Cabin")
  ) |>
  layout(
    title = titulo_plotly("¿Cuàl es el promedio de años de experiencia laboral por partido?",
...
output$plot_ranking_candidatos_sent <- renderPlotly({
  ...
  datos <- sentencias_df |>
    ...
    mutate(
      ...
      tooltip   = paste0("<b>", nombre, "</b><br>", partido, "<br>",
  ...
  plot_ly(datos,
    ...
    type = "bar", orientation = "h", marker = list(color = rojo),
    text = ~tooltip, hoverinfo = "text",
    texttemplate = ~as.character(n_sent),
    textposition = "outside", cliponaxis = FALSE,
    textfont     = list(color = blanco, size = 13, family = "Cabin")
  ) |>
  layout(
    title = titulo_plotly("¿Quiénes son los candidatos/as con más sentencias?",
                          "20 primeros, según su propia declaración · pon el cursor encima para más info"),
    xaxis = list(color = blanco, gridcolor = gris_claro,
                 dtick = 1, tickfont = list(family = "Cabin")),
    yaxis = list(color = blanco, gridcolor = "transparent",
                 tickfont = list(family = "Cabin", size = 13), tickmode = "linear", tick0 = 0, dtick = 1, automargin = TRUE, title = ""),
    paper_bgcolor = background_oscuro, plot_bgcolor = background_oscuro,
    font = list(color = blanco, family = "Cabin"),
...
# ── 8. FIXME → CORREGIDO: Ranking candidatos por ingresos ─────────────────────
# Problema anterior: n_label(fc_uniq()) se evaluaba fuera del contexto reactivo
# correcto al pasarse como argumento a titulo_plotly dentro de layout().
# Solución: calcular el label antes y guardarlo en variable local.
...
output$plot_ranking_ingresos_cand <- renderPlotly({
  ...
  label_n <- n_label(d)   # calculado aquí, dentro del contexto reactivo
...
# ── 9. Ranking sentencias por materia (top 10) ────────────────────────────────
...
output$plot_ranking_sentencias <- renderPlotly({
  ...
  layout(
    title = titulo_plotly("Ranking de sentencias por materia",
                          paste0("Top 10 · ", n_sent, " sentencias en la selección")),
    xaxis = list(color = blanco, gridcolor = gris_claro, tickfont = list(family = "Cabin")),
    yaxis = list(color = blanco, gridcolor = "transparent",
                 tickfont = list(family = "Cabin", size = 13), tickmode = "linear", tick0 = 0, dtick = 1, automargin = TRUE, title = ""),
    paper_bgcolor = background_oscuro, plot_bgcolor = background_oscuro,
...
# ── 11. Sin trayectoria · mayores ingresos (top 10) ───────────────────────────
...
output$plot_sin_trayectoria <- renderPlotly({
  ...
  layout(
    title = titulo_plotly(
      "Sin experiencia laboral, pero con altos ingresos",
      "Top 10 candidatos que no declaran exp. laboral, cargos partidarios ni elección popular declarados ante el JNE"
    ),
    xaxis = list(color = blanco, gridcolor = gris_claro,
                 tickformat = ",.0f", ticthkprefix = "S/ ",
                 tickfont = list(family = "Cabin")),
    yaxis = list(color = blanco, gridcolor = "transparent",
                 tickfont = list(family = "Cabin", size = 13), tickmode = "linear", tick0 = 0, dtick = 1, automargin = TRUE, title = ""),
    paper_bgcolor = background_oscuro, plot_bgcolor = background_oscuro,
...
output$tabla_candidatos <- DT::renderDT({
  ...
  d <- fc_tabla() |>
    transmute(
      Nombre = nombre, DNI = dni, Cargo = cargo_al_que_postula,
      Partido = partido, Departamento = departamento, Sexo = sexo,
      `Ed. Básica` = tiene_educacion_basica, `Ed. Técnica` = ed_tecnica,
      `Ed. No Univ.` = ed_no_univ, `Ed. Univ.` = ed_universitaria,
      `Ed. Posgrado` = ed_posgrado, `Exp. Laboral` = exp_laboral,
      `Cargos Part.` = cargos_partidarios, `Elec. Popular` = eleccion_popular,
      Sentencias = sentencias, Bienes = bienes,
...
for (id in c(
  "plot_ingresos_partido", "plot_sentencias_partido",
  "plot_ingreso_tipo", "plot_sexo_partido", "plot_exp_laboral",
  "plot_ranking_candidatos_sent", "plot_ranking_ingresos_cand",
  "plot_ranking_sentencias", "plot_treemap", "plot_sin_trayectoria",
  "tabla_partidos", "tabla_candidatos"
)) {
...
```
```{r}
uiOutput("vb_partidos_p")
```

## Row

```{r}
plotlyOutput("plot_sentencias_partido", height = "550px", width = "100%")
```
```{r}
...
```
```{r}
plotlyOutput("plot_ingreso_tipo", height = "550px", width = "100%")
```

## Row

```{r}
plotlyOutput("plot_sexo_partido", height = "550px", width = "100%")
```
...
```
```{r}
uiOutput("vb_sentencias_c")
```

## Row

```{r}
plotlyOutput("plot_ranking_candidatos_sent", height = "600px", width = "100%")
```
```{r}
...
```{r}
plotlyOutput("plot_ranking_ingresos_cand", height = "600px", width = "100%")
```
```{r}
plotlyOutput("plot_treemap", height = "500px", width = "100%")
```
